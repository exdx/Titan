...
class SimpleMovingAverage:
    ...
    def write_ta_statistic_to_db(self):
        ...
        with lock:
            if self.speed == 'slow':
                ins = connection_manager.TAMovingAverage.insert().values(Pair=self.market.analysis_pair,Time=self.timestamp,Close=self.close,MA_SLOW_INTERVAL=self.periods,MA_SLOW=self.sma_result)
                conn.execute(ins)
                print('Writing Slow SMA statistic to db...')
            elif self.speed == 'fast':
                ins = connection_manager.TAMovingAverage.insert().values(Pair=self.market.analysis_pair,Time=self.timestamp,Close=self.close,MA_FAST_INTERVAL=self.periods,MA_FAST=self.sma_result)
                conn.execute(ins)
    ...
    def write_strategy_description_to_db(self):
        ...
        with lock:
            ins = connection_manager.TAIdentifier.insert().values(TA_ID=1, Description='A basic SMA Crossover Strategy: {}'.format(self.speed))
            conn.execute(ins)
